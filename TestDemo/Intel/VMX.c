...
BOOLEAN VmxHardSupported()
{
	...
	/*
	void __cpuid(
	int cpuInfo[4],	//四个整数的数组，包含在 EAX、EBX、ECX 和 EDX 中返回的有关 CPU 支持的功能的信息。
	int function_id	//在 EAX 中传递的指定要检索的信息的代码。
	);
	*/

	if ((data.ecx & (1 << 5)) == 0)		//检测VMX支持 VMX支持 检测CPUID.01H:ECX[5].VMX位来确定是否支持VMX架构 1表示支持
	{
		return FALSE;
	}
	...
	// BIOS lock check
	if (Control.Fields.Lock == 0)	//lock位 为0 VMXON执行时会产生#GP异常
	{
...
VOID VmxInitializeCPU(IN PVCPU Vcpu, IN ULONG64 SystemDirectoryTableBase)
{
	 ...
	 */
	RtlCaptureContext(&Vcpu->HostState.ContextFrame);	//获取上下背景文
...
/// <summary>
/// Execute VMLAUNCH	执行VMLAUNCH	这条指令用于启动当前VMCS指针所指的一个虚拟机 并移交控制权给Guest
/// </summary>
...
/// <summary>
/// Switch CPU to root mode	转换CPU 进入ROOT模式
/// </summary>
/// <param name="Vcpu">Virtual CPU data</param>
...
BOOLEAN VmxEnterRoot(IN PVCPU Vcpu)
{
	...
	int res = __vmx_on((PULONG64)&phys);	//这条指令用于告知处理器后续可执行虚拟化相关指令  换句话说 执行该指令后就可以运行Hyperviosr
	if (res)
	{
