...
/// <summary>
/// Change protection of nonpaged system address  更改非分页地址的保护
/// </summary>
...
{
	ULONG size = 0;
	PSYSTEM_SERVICE_DESCRIPTOR_TABLE pSSDT = UtilSSDTBase();	//获取SSDT基地址 
	PVOID pBase = UtilKernelBase(&size);		//获取ntos基地址
...
{
	PUCHAR ntosBase = UtilKernelBase(NULL);	//获取的到ntos基地址
	...
	for (PIMAGE_SECTION_HEADER pSec = pFirstSec; pSec < pFirstSec + pHdr->FileHeader.NumberOfSections; pSec++)
	{
			...
			(*(PULONG)pSec->Name != 'EGAP'))
		{
			...
			{
				SSDTAddress = (PSYSTEM_SERVICE_DESCRIPTOR_TABLE)((PUCHAR)pFound + *(PULONG)((PUCHAR)pFound + 3) + 7);
...
PVOID UtilKernelBase(OUT PULONG pSize)		//根据NtOpenFile地址判断ntos模块的基地址 遍历操作系统模块
{
	NTSTATUS status = STATUS_SUCCESS;
	ULONG bytes = 0;
